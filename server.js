...
app.get('/printer-info', (req, res) => {
 ...
 const ippBody = Buffer.concat([hdr, Buffer.from([0x01]), attrs, Buffer.from([0x03])]);

 const http = require('http');
 const options = {
   hostname: PRINTER_HOST, port: 631, path: '/ipp/print', method: 'POST',
 ...
 const request = http.request(options, (response) => {
   ...
   response.on('end', () => {
     const body = Buffer.concat(chunks);
     console.log('Printer info hex:', body.toString('hex'));
     res.json({ hex: body.toString('hex'), raw: body.toString('latin1') });
   });
 });
